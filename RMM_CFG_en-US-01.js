 RMM_CFG = (function() {
    var str = {
        ...
        TXT_carry_next : 'Remember the <span style="color:blue;">carry forward</span> when adding the column.',
        ...
        MSG_sync_procdn_final_err : 'ERROR: Sync download for REPLACE_user did not complete.',
        MSG_sync_procdn_step1 : 'Downloading Data<br><span style="color:#0000b3;">(step 1 of 3)</span><br>Moment Please',
        MSG_sync_procdn_step2 : 'Downloading Data<br><span style="color:#0000b3;">(step 2 of 3)</span><br>Please Be Patient',
        MSG_sync_procdn_step3 : 'Downloading Data<br><span style="color:#0000b3;">(step 3 of 3)</span><br>Adding Records',
        MSG_sync_procdn_final_ok : 'OK: Sync download for REPLACE_user completed successfully.',
        MSG_sync_procup_step1 : 'Uploading Data<br><span style="color:#0000b3;">(step 1 of 3)</span><br>Moment Please',
        MSG_sync_procup_step2 : 'Uploading Data<br><span style="color:#0000b3;">(step 2 of 3)</span><br>Please Be Patient',
        MSG_sync_procup_step3 : 'Uploading Data<br><span style="color:#0000b3;">(step 3 of 3)</span><br>Moment Please',
        MSG_sync_procup_final_ok : 'OK: Sync upload for REPLACE_user completed successfully.',
        MSG_sync_procup_final_no_recs : 'OK: Sync upload for REPLACE_user already complete - skipping third step.',
        MSG_sync_procup_final_err : 'ERROR: Sync upload for REPLACE_user did not complete.',
        ...
        HLP_tog_notes : '<span class="help_hi">Number Position</span> shows a popup that identifies the next number position to be answered when doing multi-step addition or subtraction problems. A click is needed to advance.<br><br><span class="help_hi">Carry Forward</span> is a note shown below an addition problem that explains the carry forward. <b>Note:</b> when the carry forward note is shown, the <u>Number Position</u> popup will always show.<br><br><span class="help_hi">Borrow Needed</span> shows the "Borrowing Needed" popup when the upcoming problem is a multi-step subtraction requiring borrows. A click is needed to advance.<br><br><span class="help_hi">Borrow Explanation</span> is a note shown below a subtraction problems explaining how borrows between the ones, tens, and hundreds is done.<br><br><span class="help_hi">Chunk It</span> is information shown for a one-digit multiply problems explaining how to break the problem into its ones plus tens components.<br><br><span class="help_hi">Next Problem</span> is the popup that shows after correctly answering a problem. It allows the final solution to be viewed prior to moving on to the next problem. A click is needed to advance.<br><br><span style="font-size:90%;">Note: When starting out, it is best to show all the <u>notes &amp; popups.</u> They do, though, slow down problem solving, so try to turn them off once the process for each level of arithmetic is well-understood. <u>Chunk It</u> has value by helping to see how to simplify a problem. Be careful, though, ensure it is not over-used.</span>',
