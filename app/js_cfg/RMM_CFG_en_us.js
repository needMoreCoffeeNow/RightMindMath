 RMM_CFG = (function() {
    var str = {
        ...
        TXT_input_device : 'Please enter a simple device name like PC, Phone, or Tablet.<br>You will only do this once on this device.<br><span style="font-size:90%;">Limit entry to: a-z, A-Z, 0-9, or - [dash] <br>(20 characters max)</span>',
        TXT_input_user : 'Please enter a user name - simple names or nick-names work best.<br><span style="font-size:90%;">Limit entry to: a-z, A-Z, 0-9, or - [dash] <br>(20 characters max)</span>',
        ...
        TXT_carry_add : 'The answer is <b>REPLACE_ANSWER</b> (REPLACE_NEGATIVE<span style="font-weight:bold;color:#0000e6">REPLACE_ONES </span><span style="font-size:80%;background-color:#ddd;"> REPLACE_COLA</span> + <span style="font-weight:bold;color:#0000e6">1 </span><span style="font-size:80%;background-color:#ddd;"> REPLACE_COLB</span>).<br>The <b>REPLACE_VALUE</b> stays in the <span style="font-size:80%;background-color:#ddd;">REPLACE_COLA</span><br></b>The <b>1</b> is marked above the <span style="font-size:80%;background-color:#ddd;">REPLACE_COLB</span><br>This <span style="color:#0000e6">carry forward</span> is calculated with the <span style="font-size:80%;background-color:#ddd;">REPLACE_COLB</span>',
        ...
        TXT_sync_sheet_text : '<span style="font-size:70%;">To add this user to the <u>User ID</u> table of your Google Sheet Control tab, copy/paste the text in the box below.</span>',
        ...
        MSG_sync_process_step1 : 'Syncing Data<br><span style="color:#0000b3;">(step 1 of 7)</span><br>Moment Please',
        MSG_sync_process_step2 : 'Syncing Data<br><span style="color:#0000b3;">(step 2 of 7)</span><br>Please Be Patient',
        MSG_sync_process_step3 : 'Syncing Data<br><span style="color:#0000b3;">(step 3 of 7)</span><br>Adding Records',
        MSG_sync_process_step4 : 'Syncing Data<br><span style="color:#0000b3;">(step 4 of 7)</span><br>Please Be Patient',
        MSG_sync_process_step5 : 'Syncing Data<br><span style="color:#0000b3;">(step 5 of 7)</span><br>Moment Please',
        MSG_sync_process_step6 : 'Syncing Data<br><span style="color:#0000b3;">(step 6 of 7)</span><br>Moment Please',
        MSG_sync_process_step7 : 'Syncing Data<br><span style="color:#0000b3;">(step 7 of 7)</span><br>Moment Please',
        ...
        MSG_sync_process_data_err : 'ERROR: Sync download data for REPLACE_user was invalid.',
        ...
        MSG_user_add_done : '<span style="font-weight:bold;color:#0000e6">REPLACE_name</span> has been created, and is now the current user.<br><br>To add this user data to your Google sheet Control tab carefully copy/paste the text the text block below then paste it into Column A in the first empty row below the user ID info.<br><span style="font-size:80%;">(Note: this is also available using the <u>Show User IDs set up on this device</u> list found in the Sync Menu.)</span><br>',
        ...
        MSG_exportDB_finished : 'Data Export Complete. Look in your browser downloads folder for file:',
        ...
        MSG_stats_export_complete : ' is the file that has the stats export in the folder selected for downloads.',
        MSG_stats_export_failed : 'Something went wrong with the stats file save. Please check your browser settings to see if file saving is blocked.',
        DAT_guest : 'Guest',
    ...
    var innerHtml = {
        ...
        b_menu_subneg_save : 'Continue',
        b_menu_addneg_save : 'Continue',
        ...
        div_menu_addneg_text : 'Select how many problems with a <u>negative lower operand</u> you want for every 10 one-digit addition problems shown. <span style="font-size:80%;">Examples: 5&nbsp;+&nbsp;-7&nbsp;=&nbsp;-2  and  3&nbsp;+&nbsp;-2&nbsp;=&nbsp;1</span>. The <span style="color:#0000ff;">Current Setting</span> is:',
        div_menu_addtopneg_text : 'When a problem has a negative lower operand, the top can also be negative. Select how many times out of ten you want the problem to have a negative <u>top number</u> <span style="font-size:80%;">(e.g. -5&nbsp;+&nbsp;-3&nbsp;=&nbsp;-8)</span>. The <span style="color:#0000ff;">Current Setting</span> is:',
        div_menu_subneg_text : 'Select how many negative answers you want for every 10 one-digit subtraction problems shown. <span style="font-size:80%;">(e.g. 3&nbsp;-&nbsp;5&nbsp;=&nbsp;-2)</span> The <span style="color:#0000ff;">Current Setting</span> is:',
        div_menu_subtopneg_text : 'When a negative answer problem is presented, how many times out of ten you want the problem to have a negative <u>top number</u> <span style="font-size:80%;">(e.g. -3&nbsp;-&nbsp;8&nbsp;=&nbsp;-11)</span>. The <span style="color:#0000ff;">Current Setting</span> is:',
        ...
        div_stats_export_close_txt : '<span style="font-size:80%;"><p>CSV Export Info (check browser Download History for location)</p><hr><p><b>idname:</b> user name (note: a <b>+</b> denotes sync added user)</p><p><b>iduser:</b> the unique numeric id for the idname</p><p><b>days:</b> days since starting RMM for this session</p><p><b>idlevel:</b> problem type (a/s/m/d) followed by the number of digits in the problem</p><p><b>timestamp:</b> timestamp marking start of a problem sesssion</p><p><b>problemNum:</b> problem number for the session</p><p><b>Date:</b> session start date/time <span style="font-size:80%;"><br>(Note: select either MM/DD/YYYY or DD/MM/YYYY format)</p><p><b>time:</b> total amount of time to answer a problem</p><p><b>tries:</b> for single-digit problems tries needed (1-4) to answer correctly</p><p><b>equation:</b> equation as a delimited string</p><p><b>ordered:</b> for single-digit multiplication the value is True for ordered (not random) equation presentation such as 2x1, 2x2, 2x3, ...</p><p><b>chunked:</b> m2 problem dialog presented where <b>c</b> = Chunkit button, <b>b</b> = Basic explanation</p><hr><p>Sort the data first on <u>idlevel</u> then <u>timestamp</u> then <u>probNum</u> to chart your child&#039;s work</p></span>',
        m1_digit_txt : 'Number&nbsp;&nbsp;&nbsp;',
        ...
        b_stats_export : 'Export to CSV File',
        b_recs_export : 'Export to Json File',
        ...
        label_sync_input_pwd : 'Pass code',
